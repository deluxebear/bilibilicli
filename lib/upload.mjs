...
export async function uploadVideoMultipart(client, { file }) {
  const info = await stat(file);
  const name = basename(file);
  console.error(`upload: create multipart session for ${name} (${info.size} bytes)`);
  ...
  for (let partNumber = 1; partNumber <= totalParts; partNumber += 1) {
    console.error(`upload: request signed URL for part ${partNumber}/${totalParts}`);
    ...
    console.error(`upload: PUT part ${partNumber}/${totalParts} (${endExclusive - start} bytes)`);
  ...
  console.error("upload: complete multipart session");
...
function pickUploadUrl(data) {
  ...
  if (Array.isArray(data.reqs) && data.reqs.length > 0) {
    const put = data.reqs.find((req) => String(req.method || "PUT").toUpperCase() === "PUT" && req.url);
    return put?.url || data.reqs[0]?.url || "";
  }
...
async function uploadSignedPart({ file, uploadUrl, start, endExclusive, size, headers }) {
  ...
  try {
    ...
    await handle.read({ buffer, offset: 0, length: size, position: start });
  } finally {
    await handle.close();
  }
