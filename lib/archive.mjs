...
export async function submitArchive(client, input) {
  ...
  const body = {
    ...
    ...(input.topicId ? {
      ...
      topic_detail: {
        from_topic_id: Number(input.topicId),
        from_source: `arc.web.${input.topicFrom || "search"}`
      }
    } : {}),
    ...(input.missionId ? { mission_id: Number(input.missionId) } : {}),
    videos: [
      {
        filename: input.filename,
        title: input.videoTitle || input.title,
        desc: "",
        ...(input.subtitleLan ? { pre_subtitle: true } : {})
      }
    ],
    cover: input.cover || "",
    no_reprint: 1,
